...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx, dw, db = affine_backward(dy, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, hd in enumerate(hidden_dims, 1):
          self.params['W%d' % i] = np.random.randn(layer_input_dim, hd) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
          ...
          if self.normalization == 'batchnorm':
            ...
            X, cache = affine_bn_relu_forward(X, W, b, gamma, beta, self.bn_params[i - 1])
          elif self.normalization == 'layernorm':
            gamma = self.params['gamma%d' % i]
            beta = self.params['beta%d' % i]
            X, cache = affine_ln_relu_forward(X, W, b, gamma, beta, self.bn_params[i - 1])
          else:
        ...
        for i in range(self.num_layers - 1, 0, -1):
          ...
          if self.normalization == 'batchnorm':
            ...
            dx, dw, db, dgamma, dbeta = affine_ln_relu_backward(dx, caches[i - 1])
            grads['gamma%d' % i] = dgamma
            grads['beta%d' % i] = dbeta
          else:
